...
div
    v-data-table(
        ...
        template(v-slot:item.controls="{ item }")
            +icon_btn("list")(nuxt, :to="`/admin/content/galleries/${item.id}`")
    ...
    v-list(v-else)
        div(v-for="gallery in galleries", :key="gallery.id")
            v-list-item
                ...
                v-list-item-action
                    +icon_btn_transparent("list")(nuxt, :to="`/admin/content/galleries/${gallery.id}`")
            v-divider
    v-pagination(v-if="galleryPageCount > 1", v-model="gallery_page", :length="galleryPageCount")
</template>
...
export default class AdminContentGalleries extends Vue {
    ...
    async deleteGallery(gallery: Gallery) {
        const gallery_service = new GalleryService();
        await gallery_service.deleteById(gallery.id);
