/**
 * Separate class due to Vuex module decorator breaking down store class and essentially
 * removing all non-store-related functions. Major editor store logic is located here.
 *
...
/**
 * @constant COMPONENTS_CONFIG All globally registered components
 */
export const COMPONENTS_CONFIG: { [key: string]: ComponentConfig } = {
    UiPcContentSlide: SlideConfig,
    UiPcGridContainer: ContainerConfig,
    UiPcNavigationNavLink: NavLinkConfig,
    UiPcNavigationMenu: MenuConfig,
    UiPcGridRow: GridRowConfig,
    UiPcContentRawHtml: RawHtmlConfig,
    UiPcContentTextBlock: TextBlockConfig,
    UiPcContentGallery: GalleryConfig,
    UiPcGridColumn: GridColumnConfig,
    UiPcNavigationSectionMenu: SectionMenuConfig,
};
...
export class EditorHelper {
    /**
     * Tracking medium editor instance on the page.
     * This is required in order to prevent creating multiple instances
     * and adequately dispose of previously created one if required.
     */
    static medium_editor_instance: any = null;

    /**
     * Sets new medium editor instance. Should always be done when
     * new medium instance is created.
     * @param value
     */
    public static setMediumInstance(value: any) {
        this.medium_editor_instance = value;
    }

    /**
     * Get last created medium editor instance.
     * @returns
     */
    public static getMediumInstance() {
        return this.medium_editor_instance;
    }
     ...
     * @returns
     */
    public static getComponentsForSelector(): { [key: string]: string } {
        const result = {};
        _.each(COMPONENTS_CONFIG, (config, name) => {
            result[name] = config.selector_text;
        });
     ...
     * @param block Component tree note data (this is the raw data stored in section content)
     * @returns
     */
    public static processBlock(block: ComponentBlockType): ProcessedBlockType {
        if (block.name !== null && !COMPONENTS_CONFIG.hasOwnProperty(block.name)) {
            const error = "Unsupported component: " + block.name;
     ...
     * @param tree Current section tree reference (from editor store)
     * @returns
     */
    ...
    /**
     * Collects data from tree and blocks to form section data tree that can be saved.
     * Does the opposite to processBlock() function and creates tree note data that
     * should be saved in section content.
     * @param tree Editor tree reference
     * @param blocks Editor blocks reference
     * @returns
     */
    public static implodeSectionDataForSaving(tree: EditorTreeNode, blocks: EditorBlocks): BlockData {
        ...
        if (tree.id === null || (tree.name && COMPONENTS_CONFIG[tree.name].has_children)) {
            ...
            _.each(tree.children, (child) => {
                result.options[CHILDREN_FIELD_NAME].push(this.implodeSectionDataForSaving(child, blocks));
            });
        }
